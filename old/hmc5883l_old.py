...
class HMC5883L:
    ...
    def __init__(self, scl=1, sda=0, address=addr, gauss='1.3', declination=(0, 0)):
        self.i2c = i2c = machine.SoftI2C(scl=machine.Pin(scl, pull=machine.Pin.PULL_UP), sda=machine.Pin(sda, pull=machine.Pin.PULL_UP), freq=15000)
        print(1111, i2c.scan(), "addr: ", addr)
        # Initialize sensor.
        i2c.start()
        ...
        i2c.writeto_mem(addr, 0x00, pack('B', 0b111000))

        # Configuration register B:
        reg_value, self.gain = self.__gain__[gauss]
        i2c.writeto_mem(addr, 0x01, pack('B', reg_value))

        # Set mode register to continuous mode.
        i2c.writeto_mem(addr, 0x02, pack('B', 0x00))
        i2c.stop()
    ...
    def read(self):
        ...
        self.i2c.readfrom_mem_into(addr, 0x03, data)
